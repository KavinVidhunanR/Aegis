...
import TherapistModal from './TherapistModal.tsx';
import { AegisIcon, TrashIcon } from './Icons.tsx';
...
const ConsentToggle: React.FC<{ profile: TeenProfile; onToggle: (hasConsented: boolean) => void }> = ({ profile, onToggle }) => {
  const hasConsent = profile.consent_to_share;
  return (
...
interface ChatPageProps {
  session: Session;
  initialProfile: TeenProfile;
}

const ChatPage: React.FC<ChatPageProps> = ({ session, initialProfile }) => {
  const [profile, setProfile] = useState<TeenProfile>(initialProfile);
  ...
  useEffect(() => {
    // Fetch chats when the component mounts with a valid session
    ...
    fetchChats();
    setMode(profile.consent_to_share ? 'THERAPIST' : 'PRIVATE');

  }, [session, profile.consent_to_share]);
  ...
  const handleConsentToggle = async (hasConsented: boolean) => {
    ...
    setProfile(p => ({ ...p, consent_to_share: hasConsented }));
  ...
  return (
    <div className="flex flex-col h-screen" style={{ backgroundColor: 'var(--bg-main)' }}>
      <header className="flex items-center justify-between p-4 shadow-md z-10">
