...
export const getAegisResponse = async (userInput: string): Promise<AegisResponse> => {
  ...
  if (!apiKey) {
    throw new Error("Gemini API key not configured. Please set the API_KEY secret in your project's settings.");
  }
  ...
  try {
    ...
    if (error instanceof Error && /API key/.test(error.message)) {
      throw new Error("Your Gemini API key seems to be invalid or missing. Please check your API_KEY environment secret.");
    }
