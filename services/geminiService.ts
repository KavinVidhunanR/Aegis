...
export const getAegisResponse = async (userInput: string): Promise<AegisResponse> => {
  ...
  if (!apiKey) {
    throw new Error("Gemini API key not configured. Please set the VITE_API_KEY secret in your project's settings.");
  }
  ...
  try {
    ...
    if (typeof parsedResponse.wellbeingScore !== 'number' || parsedResponse.wellbeingScore < 0 || parsedResponse.wellbeingScore > 100) {
        parsedResponse.wellbeingScore = 50; // Default to neutral if score is invalid
    }
    ...
    if (error instanceof Error && /API key/.test(error.message)) {
      throw new Error("Your Gemini API key seems to be invalid or missing. Please check your VITE_API_KEY environment secret.");
    }
